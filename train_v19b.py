"""
v19b: Simple L1-Only Training
=============================
DenseUNet + ResNet-34 with L1 loss only (most stable approach).

Architecture: DenseUNet + ResNet-34
...
CHECKPOINT_DIR = "checkpoints/v19b"
LOG_FILE = "logs/v19b_training.log"
...
os.makedirs("checkpoints/v19b", exist_ok=True)
logging.basicConfig(
    level=logging.INFO,
...
def train_v19b():
    ...
    train_sampler = torch.utils.data.SubsetRandomSampler(list(range(n_train)))
    val_sampler = torch.utils.data.SubsetRandomSampler(list(range(n_train, len(train_ds))))
    ...
    log.info(f"v19b Training: DenseUNet + ResNet-34 (L1 ONLY)")
    log.info(f"{'='*PRINT_WIDTH}")
    log.info(f"Architecture: DenseUNet + ResNet-34 encoder")
    log.info(f"Loss: L1 ONLY (simplicity = stability)")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if avg_val_ssim > best_val_ssim:
            ...
            ckpt = os.path.join(CHECKPOINT_DIR, f"v19b_e{epoch:03d}_ssim{best_val_ssim:.4f}.pth")
