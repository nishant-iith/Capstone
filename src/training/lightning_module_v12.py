"""
v12 Module: SOTA Clinical Hybrid (Macenko + Multi-Scale + Perceptual).
The definitive model for breaking 0.75 SSIM and 25.0 dB PSNR.
"""
import torch
...
class GANModuleV12(pl.LightningModule):
    def __init__(self, gen, multi_disc, lambda_l1=300, lambda_gp=10, lambda_struct=20, lambda_percept=10):
        super().__init__()
        self.save_hyperparameters(ignore=['gen', 'multi_disc'])
        self.gen = gen
        self.disc = multi_disc # MultiScaleDiscriminator
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Adversarial Loss (Average of both scales)
        ...
        # --- Discriminator Step ---
        # 1. Local Scale
        pred_real_local, _ = self.disc(unstained, stained)
        pred_fake_local_d, _ = self.disc(unstained, fake.detach())
        loss_d_local = torch.mean(pred_fake_local_d) - torch.mean(pred_real_local)
        
        # 2. Global Scale
        _, pred_real_global = self.disc(unstained, stained)
        _, pred_fake_global_d = self.disc(unstained, fake.detach())
        loss_d_global = torch.mean(pred_fake_global_d) - torch.mean(pred_real_global)
        
        # Gradient Penalty (on local scale for stability)
    ...
    def _gradient_penalty(self, unstained, real, fake):
        ...
        d_interpolates, _ = self.disc(unstained, interpolates)
