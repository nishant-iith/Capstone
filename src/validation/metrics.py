...
def compute_metrics_batch(virtual_batch, real_batch, data_range=1.0):
    """
    Compute SSIM, PSNR, and PCC metrics for a batch of images.
    ...
    Returns:
        tuple: (ssim_scores, psnr_scores, pcc_scores) - lists of floats per image
    """
    ...
    for i in range(batch_size):
        ...
        # 3. Compute PCC (Pearson Correlation Coefficient)
        # Flatten images to calculate correlation across all pixels/channels
        v_flat = virtual_img.ravel()
        r_flat = real_img.ravel()
        pcc = np.corrcoef(v_flat, r_flat)[0, 1]
        pcc_scores.append(pcc)

    return ssim_scores, psnr_scores, pcc_scores
